...
template<typename PointerType = wchar_t>
class HostSHA256 final {
    unsigned char bytes[EVP_MAX_MD_SIZE] {};
    unsigned bytesLength = 0;
public:
    HostSHA256() = delete;
    HostSHA256(const PointerType *data, size_t length) {
        const size_t byteLength = length * (sizeof(PointerType) / sizeof(char));
    ...
    std::wstring to_wstring() const {
        ...
        for (unsigned i = 0; i < bytesLength; ++i)
            ss << std::hex << std::setw(2) << std::setfill(L'0') << static_cast<int>(bytes[i]);
        return ss.str();
    }
